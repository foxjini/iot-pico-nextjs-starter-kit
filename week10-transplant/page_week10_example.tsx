// src/app/dashboard/page.tsx ── 10주차 예시: 센서 4종 + 릴레이를 이식한 화면
"use client";
...
const SENSORS = [
  ...
  // ★ 10주차 — 이식한 센서 네 종. 우리 팀에 필요한 줄만 남기세요.
  { key: "soil",         label: "토양수분",  unit: "%",    max: 100, variant: "info"      },
  { key: "distance",     label: "거리",      unit: "cm",   max: 200, variant: "primary"   },
  { key: "dust",         label: "미세먼지",  unit: "㎍/㎥", max: 150, variant: "danger"    },
  { key: "motion_count", label: "움직임",    unit: "회",   max: 30,  variant: "secondary" },
];
...
const SWITCHES = [
  ...
  // ★ 10주차 — 이식한 액추에이터. Pico가 같은 key를 받아야 합니다.
  { key: "pump",  label: "펌프", pin: "GP20" },
];
...
// ★ 10주차 — 자동 모드가 조작할 스위치만 여기 적는다 (펌프는 사람이 직접)
...
const ALERTS = [
  ...
  // ★ 10주차 — 이식한 센서의 경고 조건
  { key: "dust", over: 75, icon: "😷", label: "미세먼지 나쁨",
    variant: "warning", hint: "창문을 닫고 공기청정기를 켜세요." },
];
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // ★ 10주차 — 자동 모드가 건드릴 스위치만 골라서 바꾼다.
    // 펌프처럼 사람이 직접 조작해야 하는 것은 건드리지 않는다.
    if (AUTO_KEYS.some((k) => sw[k] !== on)) {
      const next: SwitchState = { ...sw };
      for (const k of AUTO_KEYS) next[k] = on;
      setSw(next);
    ...
    <Container className="py-4">
      ...
      <Tabs defaultActiveKey="monitor" id="main-tabs" className="mb-3 fs-5">
        <Tab eventKey="monitor" title="실시간 모니터">
          ...
          {/* ★ 10주차 — 센서가 여섯 개를 넘으면 lg={3}으로 한 줄에 네 개를 놓는다 */}
          <Row className="g-3">
            {SENSORS.map((s) => (
              <Col key={s.key} xs={12} sm={6} lg={3}>
                <SensorCard
                  label={s.label}
            ...
            <Card.Body>
              ...
              {SWITCHES.map((s) => (
                <Form.Check
                  ...
                  className="fs-5 mb-2"
                  checked={sw[s.key]}
                  disabled={autoMode && AUTO_KEYS.includes(s.key)}
      ...
      <Modal show={showAbout} onHide={() => setShowAbout(false)} centered>
        ...
        <Modal.Body>
          ...
          <p className="mb-0">
            {/* ★ 10주차 — 우리 팀 작품 설명으로 바꾸세요 */}
            <strong>자동 모드</strong>를 켜면 주변이 어두워질 때 조명이 스스로 켜집니다.
            손으로 조도 센서를 가려 보세요.
          </p>
        </Modal.Body>
        <Modal.Footer>
